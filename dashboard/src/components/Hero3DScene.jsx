...
export default function Hero3DScene() {
  ...
  useEffect(() => {
    ...
    const keyLight = new THREE.DirectionalLight(0x10b981, 2.2); // Emerald key
    keyLight.position.set(6, 10, 8);
    keyLight.castShadow = true;
    scene.add(keyLight);

    const purpleRim = new THREE.DirectionalLight(0x8b5cf6, 1.8); // Purple rim
    purpleRim.position.set(-6, 4, -4);
    scene.add(purpleRim);

    // ── Grid Floor ──
    const grid = new THREE.GridHelper(30, 24, 0x10b981, 0x1e293b);
    grid.position.y = -0.5;
    scene.add(grid);
    ...
    // Headlights (Vibrant Emerald)
    const lightMat = new THREE.MeshStandardMaterial({ color: 0x10b981, emissive: 0x10b981, emissiveIntensity: 3.5 });
    ...
    // ── Floating Telemetry Rings (Purple) ──
    const ringGeo = new THREE.TorusGeometry(2.4, 0.03, 16, 64);
    const ringMat = new THREE.MeshBasicMaterial({ color: 0x8b5cf6, wireframe: true, transparent: true, opacity: 0.4 });
  ...
  return (
    <div
      style={{
        ...
        background: 'linear-gradient(180deg, rgba(15, 23, 40, 0.85) 0%, rgba(8, 12, 20, 0.95) 100%)',
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
          ...
          <span style={{ fontSize: '0.75rem', fontFamily: 'monospace', color: '#94a3b8', letterSpacing: '0.1em' }}>
        ...
        <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#f8fafc', margin: 0, textShadow: '0 2px 10px rgba(0,0,0,0.8)' }}>
          CBRN Chemical Hazard Unit <span style={{ color: '#8b5cf6' }}>Bay 03</span>
        </h2>
      </div>
    </div>
  );
