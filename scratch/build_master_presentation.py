...
def build_master_deck():
    ...
    # COLOR PALETTE
    # ---------------------------------------------------------
    DARK_NAVY   = RGBColor(10, 16, 30)       # #0A101E - Deep Tactical Header
    SLATE_CARD  = RGBColor(246, 249, 253)    # Crisp clean card fill
    CARD_BORDER = RGBColor(0, 120, 215)      # Tech Cyan/Blue Border
    CYAN_ACCENT = RGBColor(0, 180, 225)      # Cyan highlight
    AMBER_ALERT = RGBColor(230, 95, 0)       # Hazmat Orange / SIH Accent
    GREEN_VALID = RGBColor(16, 160, 100)     # Verified Green
    RED_HOT     = RGBColor(220, 38, 38)      # Alert Red
    TEXT_MAIN   = RGBColor(15, 23, 42)       # Slate text
    TEXT_MUTED  = RGBColor(71, 85, 105)      # Subdued text
    WHITE       = RGBColor(255, 255, 255)

    def add_header_strip(slide, title_text, category_tag="SIH 2026 // DISASTER MANAGEMENT"):
        bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(0.38), Inches(11.73), Inches(0.72))
        bar.fill.solid()
    ...
    # =========================================================
    # SLIDE 1: TITLE PAGE (MISSION INGRESS)
    # =========================================================
    s1 = prs.slides[0]
    ...
    # Hero Banner
    banner = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(0.42), Inches(11.73), Inches(1.9))
    banner.fill.solid()
    ...
    p1 = btf.paragraphs[0]
    p1.text = "SMART INDIA HACKATHON 2026 // PROBLEM STATEMENT: SIH260088"
    p1.font.size = Pt(11)
    p1.font.bold = True
    ...
    # Left: Metadata Card
    meta_card = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(2.48), Inches(5.6), Inches(4.35))
    ...
    mp = mtf.paragraphs[0]
    mp.text = "📋 MISSION SPECIFICATIONS & TASK FORCE"
    mp.font.size = Pt(13)
    mp.font.bold = True
    ...
    meta_rows = [
        ...
        ("Target Ministry", "Disaster Management (NDRF / MHA)"),
        ("Solution Category", "Software & Enterprise Telemetry"),
        ("Lead Institution", "Kalpataru Institute of Technology (KIT)"),
        ("Team Lead", "Lohith R C (Unity Physics, Backend & Telemetry)"),
        ("Task Force Roster", "Monica K S, Chandana M P, Chandana M N, Harshini R B, Pavitra J H")
    ]
    ...
    if os.path.exists(hero_img):
        s1.shapes.add_picture(hero_img, Inches(6.6), Inches(2.48), Inches(5.93), Inches(4.35))
        
        hud_tag = s1.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(6.8), Inches(6.38), Inches(5.53), Inches(0.35))
        hud_tag.fill.solid()
    ...
    pillars = [
        ("🚨 The Real-World Crisis", [
            ("Lethal Hazard Risks:", "Live CBRN drills risk fatal toxic vapor & radiation exposure."),
            ("Prohibitive Suit Cost:", "Single-use Level-A suits cost ₹50,000+ per drill."),
            ("Zero Muscle Memory:", "Passive classroom lectures fail to build real reflex."),
            ("Subjective Bias:", "Human evaluations miss microsecond protocol errors.")
        ], RED_HOT, RGBColor(255, 245, 245)),
        
        ("☣️ The CBRS-X Solution", [
            ("Zero-Risk 3D Simulation:", "Virtual recreation of Storage Bay 03 hot zone."),
            ("Zero-Install WebGL:", "Runs natively in Chrome/Edge on budget laptops."),
            ("Strict SOP Enforcement:", "Governs official 5-stage NDRF emergency cycle."),
        ...
        ("🌟 Core Innovation & Edge", [
            ("Dual-Screen Sync:", "WebGL Trainee view synced live with Instructor Radar."),
            ("Sub-50ms Telemetry:", "STOMP WebSockets streaming responder vitals at 60Hz."),
    ...
    for idx, (head, items, border_c, bg_c) in enumerate(pillars):
    ...
    add_header_strip(s3, "TECHNICAL APPROACH & SYSTEM ARCHITECTURE // MULTI-TIER ENGINE", "FULL-STACK SPECS")
    ...
    # Left: Technical Feasibility Box
    feat_card = s4.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.25), Inches(5.5), Inches(4.95))
    feat_card.fill.solid()
    feat_card.fill.fore_color.rgb = RGBColor(245, 248, 255)
    feat_card.line.color.rgb = CARD_BORDER
    feat_card.line.width = Pt(1.5)
    
    ftf = feat_card.text_frame
    ftf.margin_left = Inches(0.2)
    ftf.margin_top = Inches(0.18)
    
    fp = ftf.paragraphs[0]
    fp.text = "📦 TECHNICAL & OPERATIONAL FEASIBILITY"
    fp.font.size = Pt(12.5)
    fp.font.bold = True
    fp.font.color.rgb = DARK_NAVY
    ...
    fpoints = [
        ...
        ("Local Hotspot Connectivity", "Responders connect laptops/tablets to a local router with zero external internet required."),
        ...
        ("Multi-Hazard Polymorphism", "Hot-swappable JSON state machines for Chemical, Biological, and Radiological disaster scenarios.")
    ]
    ...
    # Right: Risk Analysis & Mitigation Matrix
    risk_card = s4.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.5), Inches(1.25), Inches(6.03), Inches(4.95))
    ...
    risk_items = [
        ...
        ("No Internet at Field Outposts", "Offline 'Field Box' Deployment: Entire ecosystem packaged in Docker Compose running on local offline Wi-Fi router."),
        ...
        ("Cybersecurity & Ingress Risks", "Bounded LRU Sliding-Window Rate Limiting (10k IP pool) + DDE / CSV injection sanitization filters.")
    ]
    ...
    # =========================================================
    # SLIDE 6: RESEARCH BASE & REFERENCES
    # =========================================================
    s6 = prs.slides[5]
    ...
    out_path = "CBRS_X_SIH_2026_Master_Presentation.pptx"
    prs.save(out_path)
    print(f"Master presentation saved successfully: {out_path}")
